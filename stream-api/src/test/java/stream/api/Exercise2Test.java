...
public class Exercise2Test extends ClassicOnlineStore {
    ...
    public void descSortByAge() {
         ...
         */
        Comparator<Integer> descOrder = (int1, int2) -> {
        	return int2.compareTo(int1);
        };
        //Stream<Integer> sortedAgeStream = customerList.stream().map(Customer::getAge).sorted(descOrder);
        Stream<Integer> sortedAgeStream = customerList.stream().map(Customer::getAge).sorted((int1, int2) -> int2.compareTo(int1));
    ...
    public void top3RichCustomer() {
		...
		Stream<String> top3RichCustomerStream = customerList
				.stream()
				.sorted((customer1, customer2) -> customer2.getBudget()
						.compareTo(customer1.getBudget()))
    ...
    public void itemsCustomersWantToBuy() {
        ...
        //Stream<String> itemStream = customerList.stream().map(Customer::getWantToBuy).flatMap(items -> items.stream().map(Item::getName));
        Stream<String> itemStream = customerList.stream().flatMap((customer) -> customer.getWantToBuy().stream()).map(Item::getName);
